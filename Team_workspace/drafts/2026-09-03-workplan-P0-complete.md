...
## Phase 1 — מנוע החיתוך (5 משימות) — ✅ **פתוח (unblocked)**, ⏳ **לא בוצע**

§0 נפתר ל-GPL ב-2026-09-03. **אף אחת מהמשימות 1.1–1.5 לא בוצעה בפועל.** קיים שלד תיקיות ריק/סמלי תחת `src/plugins/PluginHost/plugins/` (arcane-engine עם manifest.json+GPL, infill-library, supports-engine, wipe-tower) — אבל אלו נראים כמו scaffolding, לא מימוש אמיתי (לא נבדק לעומק ב-session הזה). **צריך בדיקת סטטוס אמיתי לפני שממשיכים.**
...
| §0 לא מתקבל לפי לוח הזמנים | — | — | **נפתר, לא רלוונטי יותר** |
...
1. **🔴 דחוף — להכריע בכפילות arcane-engine.** לפני שממשיכים ל-Phase 1 בכלל, להחליט: המימוש הקנוני הוא C# (`kuziSlicer.PluginHost`, כמתוכנן ב-§0) או TS (`kuziSlicer.extensions`, מה שנדחף בפועל)? למחוק/לארכב את השני כדי למנוע בלבול ועבודה כפולה. אם הקוד ב-TS נשאר — לבדוק עם ייעוץ משפטי אם זה עומד בדרישת הבידוד של §0.

2. **החלטה: Klipper/Moonraker נדרש ל-P0?** אם כן — 3.2 עדיין פתוח לביצוע במלואו (לא נבנה כלל). אם לא — לעדכן את ה-PRD/HLD שהיעד השתנה ל"Bambu + Elegoo כמתאמים הראשונים ב-P0", ולתעד את זה רשמית (לא רק כאן).
